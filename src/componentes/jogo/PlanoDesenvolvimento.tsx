...
export function PlanoDesenvolvimento({ carreira: c }: { carreira: EstadoCarreira }) {
  ...
  return <section className="painel espaco"><h2>PLANO DE DESENVOLVIMENTO</h2>
    ...
    {ultimo && <p className="citacao">Último treino: {ultimo.avaliacao}. Confiança {ultimo.confianca >= 0 ? '+' : ''}{ultimo.confianca.toFixed(1)}. {ultimo.progresso > 0 ? 'Trabalho acumulado nos fundamentos.' : 'Semana dedicada à recuperação.'}</p>}
  </section>;
}
